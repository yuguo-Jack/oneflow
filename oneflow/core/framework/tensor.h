...
class Parameter final : public TensorIf<Parameter> {
 public:
  ...
  Maybe<ConsistentTensor> AsConsistentTensor() override {
    if (const auto& conssitent_tensor = std::dynamic_pointer_cast<ConsistentTensor>(tensor_)) {
      return conssitent_tensor;
    }
    UNIMPLEMENTED_THEN_RETURN();
  }
...
class ConsistentTensor final : public TensorIf<ConsistentTensor>, public std::enable_shared_from_this<ConsistentTensor> {
 public:
