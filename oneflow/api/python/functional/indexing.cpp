...
Maybe<Tensor> ConvertToIndexingTensor(PyObject* object) {
  ...
  if (PyArray_Check(object)) {
    auto tensor = TensorWithData(object, NullOpt, device, false);
    if (dtype != DataType::kInt64 || dtype != DataType::kInt32) {
      // reserved for future Casting support possibility
      // return Cast(JUST(tensor), JUST(DType::Get(DataType::kInt64)));
      return Error::RuntimeError() << "Ony support int32/int64 as index!";
    } else {
      return tensor;
    }
  }
