...
def parse_option():
    ...
    parser.add_argument("--run_iters", type=int, default=100, help="cal time iters")
    parser.add_argument(
        "--min_size_for_each_dim", type=int, default=100, help="random size range"
    )
    parser.add_argument(
        "--max_size_for_each_dim", type=int, default=200, help="random size range"
    )
    parser.add_argument("--einsum_testcase", type=str, help="random size range")
...
# pytorch
torch_input_tensors = [torch.rand(*shape, device="cuda") for shape in input_shapes]
for i in range(args.warm_up_iters + args.run_iters):
    if i == args.warm_up_iters:
        start_time = time.time()
    out = torch.einsum(equation, *torch_input_tensors)
    out_np = out.cpu().numpy()
total_time_ms = (time.time() - start_time) * 1000
torch_time_per_run_ms = total_time_ms / args.run_iters
print(
    f"pytorch avg time per iter: {torch_time_per_run_ms:.2f} ms (= {total_time_ms:.1f}ms / {args.run_iters})"
)

relative_speed = torch_time_per_run_ms / oneflow_time_per_run_ms
print(
    f"Relative speed: {relative_speed:.2f} (= {torch_time_per_run_ms:.2f}ms / {oneflow_time_per_run_ms:.2f}ms)\n"
)
